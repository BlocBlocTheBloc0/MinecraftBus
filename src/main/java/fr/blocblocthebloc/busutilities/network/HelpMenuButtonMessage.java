...
public class HelpMenuButtonMessage {
	private final int buttonID, x, y, z;

	public HelpMenuButtonMessage(FriendlyByteBuf buffer) {
	...
	public HelpMenuButtonMessage(int buttonID, int x, int y, int z) {
	...
	public static void buffer(HelpMenuButtonMessage message, FriendlyByteBuf buffer) {
	...
	public static void handler(HelpMenuButtonMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HelpMenuMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			LignesAccesProcedure.execute(entity);
		}
		if (buttonID == 1) {
