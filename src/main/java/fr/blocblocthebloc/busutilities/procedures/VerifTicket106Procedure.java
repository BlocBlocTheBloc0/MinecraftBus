...
public class VerifTicket106Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(BusutilitiesModItems.TICKET_10_VOYAGE_6.get())) : false) {
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(BusutilitiesModItems.TICKET_10_VOYAGE_5.get());
				_setstack.setCount(1);
				ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
			}
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(BusutilitiesModItems.TICKET_10_VOYAGE_6.get());
			...
			BusutilitiesModVariables.MapVariables.get(world).last_ticket_use_text = "Ticket type : 10V";
			BusutilitiesModVariables.MapVariables.get(world).syncData(world);
			...
			if (entity instanceof Player _player && !_player.level.isClientSide())
				_player.displayClientMessage(Component.literal("\u00A7aTitre de transport valide - 5 Voyages Restants"), (true));
		} else {
			VerifTicket105Procedure.execute(world, entity);
		}
	}
}
